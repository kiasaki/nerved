...
func (a *App) runLoop() error {
  ...
  for {
    ...
    case key.Event:
      ...
      if e.Name == "W" && e.Modifiers.Contain(key.ModCommand) {
...
func (a *App) Update() {
  ...
  if len(a.termEditor.Events()) > 0 {
    ...
    if selS == selE && len(t) > 0 && t[max(0, selS-1)] == '\n' && selS != a.termLast {
...
func (a *App) loadDir(d string) {
  ...
  check(err)
  title := "nerved " + strings.Replace(a.dir, envHome, "~", -1)
...
func (a *App) termRun(commandText string) {
  if a.termCmd != nil {
    syscall.Kill(-a.termCmd.Process.Pid, syscall.SIGKILL)
  }
